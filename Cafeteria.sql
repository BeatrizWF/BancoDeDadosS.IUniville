	...
	ALTER TABLE PEDIDOS ADD CONSTRAINT CLIENTEFK FOREIGN KEY (PEDIDO_CLIENTEFK) 
	REFERENCES CLIENTES (ID_CLIENTE_CPF);
	...
	ALTER TABLE ITEMCAFE ADD CONSTRAINT ITEMPRODUTOFK FOREIGN KEY (ITEM_PRODUTOFK)
	REFERENCES PRODUTOS (ID_PRODUTO);
	ALTER TABLE ITEMCAFE ADD CONSTRAINT ITEMINSUMOFK FOREIGN KEY (ITEM_INSUMOFK)
	...
	CREATE VIEW ESTOQUE AS SELECT INSUMO_NOME as 'Produto', INSUMO_QUANTIDADE as
	'Quantidade', INSUMO_UNIDADEMEDIDA as 'UnidadeMedida' FROM INSUMOS;
	...
	CREATE PROCEDURE cadastrarCliente (@CPF INT, @NOME VARCHAR(50),
	@ENDERECO VARCHAR(100), @TELEFONE INT)
	AS
	BEGIN
	    INSERT INTO CLIENTES (ID_CLIENTE_CPF, NOME, ENDERECO, TELEFONE)
	...
	BEGIN
		...
		SET @VALORTOTAL = (SELECT sum(item.QUANTIDADE_ITEM_PRODUTO * 
		prod.PRODUTO_PRECOUNIT) FROM ITEMPEDIDOS AS item join PRODUTOS
		AS prod ON item.PRODUTOFK_ITEM = prod.ID_PRODUTO WHERE PEDIDOFK_ITEM = 1);
	...
	EXEC cadastrarCliente @CPF = 50, @NOME = 'cesar', 
	@ENDERECO = 'testedeendereco', @TELEFONE = 216;
	EXEC procuraCliente @CPF = 50; 

	EXEC cadastrarCliente @CPF = 5461, @NOME = 'Chris',
	@ENDERECO = 'endereco2teste', @TELEFONE = 4710;
	EXEC procuraCliente @CPF = 5461;

	EXEC cadastrarCliente @CPF = 546111, @NOME = 'Ruan',
	@ENDERECO = 'endereco3teste', @TELEFONE = 522221;
